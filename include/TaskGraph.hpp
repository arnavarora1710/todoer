...
struct TaskGraph
{
    ...
    std::unordered_map<std::string, std::variant<int, double>> &m_variables;

    TaskGraph() = delete;
    // only need to construct from an expression
    TaskGraph(Expression &expr, std::unordered_map<std::string, std::variant<int, double>> &variables);
